...
Before launching the tuning procedure, the assessment reference files must already be available. These files are derived from the landings and assessment datasets that have been previously converted into ALADYM-compatible format. No conversion or preprocessing of assessment data is performed at this stage: the tuning procedure assumes that the assessment SSB, Fbar, and catch time series are already present and correctly formatted.

Once all Tr–Fbar–specific configuration files have been generated, the tuning phase is launched using:

`ALADYM_tuning.R`

This script orchestrates the full calibration workflow. It iterates sequentially over all Tr–Fbar configuration files created in the previous step and launches ALADYM in batch mode for each configuration without requiring user interaction.

For every Tr–Fbar combination, the script extracts annual time series of spawning stock biomass (SSB), Fbar, and total catch from the ALADYM outputs. These simulated quantities are then compared against the corresponding assessment data. Relative errors are computed, and a scalar score is calculated for each combination as the sum of mean squared relative errors across all indicators.

## Outputs and diagnostics

All results are accumulated into a single calibration table that links each Tr–Fbar combination to its diagnostic metrics and overall score. In parallel, the script creates a dedicated output folder for each parameter combination. Each folder contains the full set of diagnostic plots and intermediate results associated with that specific run, allowing post hoc inspection of individual simulations.
...
All output folders and files generated by ALADYM_tuning.R are written to the same directory in which the tuning script resides. No automatic cleanup is performed. To maintain an organised project structure, it is recommended to manually create a species-specific folder and move all tuning outputs into that location once the procedure has completed.
