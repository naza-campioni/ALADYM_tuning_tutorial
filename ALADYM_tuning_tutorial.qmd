...
1. Ensure BEMTOOL and ALADYM are already installed and functioning.

2. Download the tuning repository from GitHub, either by:

    - cloning with git clone <repository_link>, or

    - downloading the repository as a ZIP file and extracting it.

3. Place the downloaded folder in a working directory of your choice (it does not need to overwrite BEMTOOL).

4. Open R or RStudio.

5. Set the working directory to the root of the tuning repository.

6. Open ALADYM_tuning.R (or the selectivity tuning script).

7. After generating the configuration files with the provided main configuration script, run the tuning by executing source("ALADYM_tuning.R").

The tuning scripts assume that BEMTOOL is already correctly configured and that ALADYM can run in batch mode. No additional installation steps are required beyond placing the repository locally and ensuring that internal source paths match the local BEMTOOL directory structure.
...
Assessment reference files (e.g. SSB, Fbar, catch time series) must correspond to the same stock and spatial unit.

The required Excel templates and example input files are provided in the distributed package. These files illustrate the correct ALADYM format and differ from the standard BEMTOOL input structure. Users should not attempt to reuse BEMTOOL landings files directly without restructuring them into the species-specific ALADYM format.
