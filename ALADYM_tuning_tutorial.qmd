...
author: "Isabella Bitetto, Nazareno Campioni"
format: html
editor: 
  markdown:
    wrap: 72
...
Since the goal of the tuning process is to compare ALADYM outputs to stock assessment outputs, Fbar, SSB and catches `.csv` files need to be provided in the main ALADYM tuning folder (eg, `ALADYM-ver.Tuning`). They *necessarily* need to be saved as `_assessment.csv`; since each stock is run independently, ALADYM will not check whether the assessment files correspond correctly to the analysed species, hence the suggestion is to save each assessment file with the species name in it as shown in the examples below:

<a href="data/catches_assessment_NEP.csv" download>
Catches csv example
</a>

<a href="data/ssb_assessment_MUT.csv" download>
SSB csv example
</a>

<a href="data/fbar_assessment_DPS.csv" download>
Fbar csv example
</a>
...
If not done previously, the selectivity files will need to be modified as follows: a new column named `type` needs to be created and the metier type needs to be specified - that is, `HOK`, `DTS` etc.
...
Since we assume that selectivity is similar across metiers, the configuration-generation step begins by defining a mapping between metier types and the parameters they are allowed to use. This mapping is implemented as a hash map where:

- Each key represents a metier identifier.

- Each value is the set of parameter names that are active for that function.

Since each metier could be assigned a different selectivity type based on the target species, it is necessary to provide a selectivity configuration per species. In this example, we configure the selectivity for European hake for GSAs 17-18 where the involved metiers have either selectivity 2 or selectivity 6. Note that, for example, to configure the selectivity for deep-water rose shrimp, the same metier DTS will have a selectivity of type 1 and therefore the number of parameters to provide will be different. For details on the selectivity types, consult the [BEMTOOL official guide](https://coispa.github.io/BEMTOOL_tutorial_SEAwiseWP6/BEMTOOL_tutorialMSE.html).
...
These parameters control the shape of the function associated with each selectivity type; for example, HOK is given a selectivity type of 6 which corresponds to a dome-shaped function and consequently its parameters control peak, age and curvature. Although the input file contains a fixed number of parameter columns, only the parameters listed in the mapping for the selected type are considered active. The remaining parameter slots are ignored during combination generation and remain fixed at their baseline values.
